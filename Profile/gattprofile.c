...
#define SIMPLEPROFILE_CHAR3_VALUE_POS    6
...
static uint8_t simpleProfileChar1Props = GATT_PROP_READ;

// Characteristic 1 Value
static uint8_t simpleProfileChar1[SIMPLEPROFILE_CHAR_TXT_LEN] = {0};

// Simple Profile Characteristic 2 Properties
static uint8_t simpleProfileChar2Props = GATT_PROP_WRITE;

// Characteristic 2 Value
static uint8_t simpleProfileChar2[SIMPLEPROFILE_CHAR_CMD_LEN] = {0};

// Simple Profile Characteristic 3 Properties
static uint8_t simpleProfileChar3Props = GATT_PROP_NOTIFY;

// Characteristic 3 Value
static uint8_t simpleProfileChar3[SIMPLEPROFILE_CHAR_ANS_LEN] = {0};
...
bStatus_t SimpleProfile_SetParameter(uint8_t param, uint16_t len, void *value)
{
    ...
    switch(param)
    {
        case SIMPLEPROFILE_CHAR_TXT:
            if(len == SIMPLEPROFILE_CHAR_TXT_LEN)
            {
                tmos_memcpy(simpleProfileChar1, value, SIMPLEPROFILE_CHAR_TXT_LEN);
            }
            else
            {
                ret = bleInvalidRange;
...
bStatus_t SimpleProfile_GetParameter(uint8_t param, void *value)
{
    ...
    {
        case SIMPLEPROFILE_CHAR_TXT:
            tmos_memcpy(value, simpleProfileChar1, SIMPLEPROFILE_CHAR_TXT_LEN);
            break;

        case SIMPLEPROFILE_CHAR_ANS:
            tmos_memcpy(value, simpleProfileChar3, SIMPLEPROFILE_CHAR_ANS_LEN);
            break;
    ...
    if(pAttr->type.len == ATT_BT_UUID_SIZE)
    {
        ...
        switch(uuid)
        {
            ...
            case SIMPLEPROFILE_CHAR1_UUID:
                if(maxLen > SIMPLEPROFILE_CHAR_TXT_LEN)
                {
                    *pLen = SIMPLEPROFILE_CHAR_TXT_LEN;
                }
                else
                {
                    *pLen = maxLen;
                ...
                extern void upd_txt(void *bfr, unsigned sz);
                upd_txt(simpleProfileChar1, SIMPLEPROFILE_CHAR_TXT_LEN);

                tmos_memcpy(pValue, pAttr->pValue, *pLen);
                break;

            case SIMPLEPROFILE_CHAR2_UUID:
                if(maxLen > SIMPLEPROFILE_CHAR_CMD_LEN)
                {
                    *pLen = SIMPLEPROFILE_CHAR_CMD_LEN;
                }
                else
                {
                    *pLen = maxLen;
            ...
            case SIMPLEPROFILE_CHAR3_UUID:
                if(maxLen > SIMPLEPROFILE_CHAR_ANS_LEN)
                {
                    *pLen = SIMPLEPROFILE_CHAR_ANS_LEN;
                }
                else
                {
                    *pLen = maxLen;
    ...
    if(pAttr->type.len == ATT_BT_UUID_SIZE)
    {
        ...
        switch(uuid)
        {
            case SIMPLEPROFILE_CHAR2_UUID:
                ...
                if(offset == 0)
                {
                    if(len > SIMPLEPROFILE_CHAR_CMD_LEN)
                    {
                ...
                if(status == SUCCESS)
                {
                    tmos_memcpy(pAttr->pValue, pValue, SIMPLEPROFILE_CHAR_CMD_LEN);
                    notifyApp = SIMPLEPROFILE_CHAR_CMD;
                }
                break;
