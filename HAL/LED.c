...
typedef struct
{
    ...
    uint16_t active_ms; /* On cycle, ms */
    uint16_t period_ms;  /* On/off cycle time (msec) */
 ...
 * @return  none
 */
void HalLedBlink(uint8_t leds, uint8_t numBlinks, uint16_t active_ms, uint16_t period_ms)
{
    uint8_t          led;
    HalLedControl_t *sts;

    if(leds && active_ms && period_ms)
    {
        if(active_ms < period_ms)
        {
            ...
            while(leds)
            {
                if(leds & led)
                {
                    ...
                    sts->period_ms = MS1_TO_SYSTEM_TIME(period_ms);           /* Time for one on/off cycle */
                    sts->active_ms = MS1_TO_SYSTEM_TIME(active_ms);         /* LED is on */
            ...
            if(leds & led)
            {
                if(sts->mode & HAL_LED_MODE_BLINK)
                {
                    ...
                    if(time >= sts->next)
                    {
                        ...
                        {
                            dur = sts->period_ms - sts->active_ms;  /* Cycle for off */
                        ...
                        {
                            dur = sts->active_ms;              /* Cycle for on */
